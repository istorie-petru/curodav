...
class TestTaskCrudNeverTouchesBridge:
    def test_create_task_writes_straight_to_sql(self, conn):
        ...
        tasks_router.create_task(title="Buy milk", description="", due_at="", priority="", status="active",
...
class TestExplodingBridgeNeverInvoked:
    ...
    def test_task_flow_with_exploding_bridge_object_floating_nearby(self, conn):
        ...
        tasks_router.create_task(title="X", description="", due_at="", priority="", status="active",
                                  tags="", recurrence="", parent_uid="", conn=conn)
        uid = db.list_tasks(conn)[0]["uid"]
        tasks_router.update_task(uid, title="Y", description="", due_at="", start_at="", priority="",
