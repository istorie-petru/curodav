"""Release 1.1 (virtual & derived states, plans/open-priority.md § Virtual &
derived states) -- slices 1-5: the Importance/Urgency data model, label rules,
the pure effective-value module, and the two-axis UI that replaced the old
single priority.

Covers:
  1. Schema: tasks.importance / tasks.urgency columns exist (both for a fresh
     database and as _ensure_column migrations on an existing one), survive
     upsert_task round-trips, and the old `priority` column stays physically
     present.
  2. label_config: the `importance` / `urgency_threshold_days` behavior keys
     upsert and read back through effective_label_config with defaults.
  3. src/derived_state.py: deterministic effective-importance / effective-
     urgency derivation from explicit values + label rules + temporal state,
     with max precedence and no stored derived state.
  4. Slice 5: the two axes in the UI -- independent filters, sort keys,
     inline field edits, and table/board/detail rendering.
"""

from __future__ import annotations

from datetime import date, datetime, timedelta, timezone

import pytest

from icalendar import Todo

from src import db, derived_state, ical_rows
from src.ical_rows import ical_to_task_row, task_row_to_ical
from src.routers import tasks as tasks_router
from src.routers import timeline as timeline_router


@pytest.fixture()
def conn(tmp_path):
    db_path = tmp_path / "cache.sqlite"
    with db.connect(db_path) as c:
        yield c


def _now() -> str:
    return datetime.now(timezone.utc).isoformat()


def _seed_task(conn, uid, due_at=None, importance=None, urgency=None, tags=None, status="active"):
    db.upsert_task(
        conn,
        {
            "uid": uid,
            "title": uid,
            "description": "",
            "status": status,
            "due_at": due_at,
            "importance": importance,
            "urgency": urgency,
            "tags": tags or [],
            "created_at": _now(),
        },
    )


def _rules(conn, *labels):
    return {name: db.effective_label_config_ci(conn, name) for name in labels}


class TestSchema:
    def test_fresh_database_has_importance_and_urgency_columns(self, conn):
        cols = {row[1] for row in conn.execute("PRAGMA table_info(tasks)").fetchall()}
        assert "importance" in cols
        assert "urgency" in cols
        # The old WebDAV priority column stays physically present (never
        # force-dropped, same convention as every other removal in db.py).
        assert "priority" in cols

    def test_existing_database_gains_columns_via_migration(self, tmp_path):
        db_path = tmp_path / "cache.sqlite"
        with db.connect(db_path) as c:
            # Simulate a pre-1.1 database: drop the two new columns, exactly
            # the on-disk state an existing install upgrading into 1.1 has.
            c.execute("ALTER TABLE tasks DROP COLUMN importance")
            c.execute("ALTER TABLE tasks DROP COLUMN urgency")
            c.execute("ALTER TABLE label_config DROP COLUMN importance")
            c.execute("ALTER TABLE label_config DROP COLUMN urgency_threshold_days")
        with db.connect(db_path) as c:  # reconnect -> init_schema runs migrations
            cols = {row[1] for row in c.execute("PRAGMA table_info(tasks)").fetchall()}
            assert "importance" in cols
            assert "urgency" in cols
            lcols = {row[1] for row in c.execute("PRAGMA table_info(label_config)").fetchall()}
            assert "importance" in lcols
            assert "urgency_threshold_days" in lcols

    def test_upsert_round_trips_importance_and_urgency(self, conn):
        _seed_task(conn, "t1", importance=3, urgency=2)
        t = db.get_task(conn, "t1")
        assert t["importance"] == 3
        assert t["urgency"] == 2
        # An edit that doesn't mention the axes leaves them alone (round-trip
        # pattern carries existing values through).
        row = dict(t)
        row["title"] = "renamed"
        db.upsert_task(conn, row)
        t2 = db.get_task(conn, "t1")
        assert t2["importance"] == 3
        assert t2["urgency"] == 2
        assert t2["title"] == "renamed"

    def test_unset_defaults_to_none(self, conn):
        _seed_task(conn, "t1")
        t = db.get_task(conn, "t1")
        assert t["importance"] is None
        assert t["urgency"] is None


class TestLabelConfigRules:
    def test_rules_upsert_and_read_back(self, conn):
        db.upsert_label_config(
            conn,
            {
                "name": "Exam",
                "importance": 3,
                "urgency_threshold_days": 7,
                "created_at": _now(),
            },
        )
        cfg = db.effective_label_config(conn, "Exam")
        assert cfg["importance"] == 3
        assert cfg["urgency_threshold_days"] == 7

    def test_rule_defaults_when_unset(self, conn):
        cfg = db.effective_label_config(conn, "Exam")
        assert cfg["importance"] is None
        assert cfg["urgency_threshold_days"] is None

    def test_partial_upsert_keeps_other_fields(self, conn):
        db.upsert_label_config(
            conn,
            {
                "name": "Exam",
                "color": "red",
                "importance": 2,
                "created_at": _now(),
            },
        )
        db.upsert_label_config(conn, {"name": "Exam", "urgency_threshold_days": 14, "created_at": _now()})
        cfg = db.effective_label_config(conn, "Exam")
        assert cfg["color"] == "red"  # untouched by the second upsert
        assert cfg["importance"] == 2
        assert cfg["urgency_threshold_days"] == 14


class TestEffectiveImportance:
    def test_explicit_only(self, conn):
        _seed_task(conn, "t1", importance=2)
        assert derived_state.effective_importance(db.get_task(conn, "t1"), {}) == 2

    def test_none_when_nothing_says_importance(self, conn):
        _seed_task(conn, "t1")
        assert derived_state.effective_importance(db.get_task(conn, "t1"), {}) == 0

    def test_label_rule_sets_importance(self, conn):
        db.upsert_label_config(conn, {"name": "Exam", "importance": 3, "created_at": _now()})
        _seed_task(conn, "t1", tags=["Exam"])
        assert derived_state.effective_importance(db.get_task(conn, "t1"), _rules(conn, "Exam")) == 3

    def test_max_precedence_between_explicit_and_label(self, conn):
        db.upsert_label_config(conn, {"name": "Exam", "importance": 3, "created_at": _now()})
        _seed_task(conn, "t1", importance=2, tags=["Exam"])
        assert derived_state.effective_importance(db.get_task(conn, "t1"), _rules(conn, "Exam")) == 3
        _seed_task(conn, "t2", importance=3, tags=["Exam"])
        assert derived_state.effective_importance(db.get_task(conn, "t2"), _rules(conn, "Exam")) == 3

    def test_max_across_multiple_labels(self, conn):
        db.upsert_label_config(conn, {"name": "A", "importance": 1, "created_at": _now()})
        db.upsert_label_config(conn, {"name": "B", "importance": 3, "created_at": _now()})
        _seed_task(conn, "t1", tags=["A", "B"])
        assert derived_state.effective_importance(db.get_task(conn, "t1"), _rules(conn, "A", "B")) == 3

    def test_label_without_rule_contributes_nothing(self, conn):
        _seed_task(conn, "t1", tags=["Exam"])  # no label_config row at all
        assert derived_state.effective_importance(db.get_task(conn, "t1"), {}) == 0


class TestEffectiveUrgency:
    def test_explicit_only(self, conn):
        _seed_task(conn, "t1", urgency=2)
        assert derived_state.effective_urgency(db.get_task(conn, "t1"), {}, date(2026, 1, 10)) == 2

    def test_label_threshold_implies_urgency_within_window(self, conn):
        db.upsert_label_config(conn, {"name": "Conference", "urgency_threshold_days": 7, "created_at": _now()})
        today = date(2026, 1, 10)
        _seed_task(conn, "inside", due_at=(today + timedelta(days=3)).isoformat(), tags=["Conference"])
        _seed_task(conn, "outside", due_at=(today + timedelta(days=30)).isoformat(), tags=["Conference"])
        _seed_task(conn, "nodate", tags=["Conference"])
        rules = _rules(conn, "Conference")
        assert derived_state.effective_urgency(db.get_task(conn, "inside"), rules, today) == 3
        assert derived_state.effective_urgency(db.get_task(conn, "outside"), rules, today) == 0
        assert derived_state.effective_urgency(db.get_task(conn, "nodate"), rules, today) == 0

    def test_temporal_urgency_overdue_and_today(self, conn):
        today = date(2026, 1, 10)
        _seed_task(conn, "overdue", due_at=(today - timedelta(days=1)).isoformat())
        _seed_task(conn, "today", due_at=today.isoformat())
        _seed_task(conn, "tomorrow", due_at=(today + timedelta(days=1)).isoformat())
        _seed_task(conn, "later", due_at=(today + timedelta(days=10)).isoformat())
        assert derived_state.effective_urgency(db.get_task(conn, "overdue"), {}, today) == 3
        assert derived_state.effective_urgency(db.get_task(conn, "today"), {}, today) == 3
        assert derived_state.effective_urgency(db.get_task(conn, "tomorrow"), {}, today) == 2
        assert derived_state.effective_urgency(db.get_task(conn, "later"), {}, today) == 0

    def test_max_precedence_across_sources(self, conn):
        db.upsert_label_config(conn, {"name": "Conference", "urgency_threshold_days": 7, "created_at": _now()})
        today = date(2026, 1, 10)
        # Explicit low, label window + temporal both push higher -- max wins.
        _seed_task(conn, "t1", urgency=1, due_at=(today + timedelta(days=3)).isoformat(), tags=["Conference"])
        assert derived_state.effective_urgency(db.get_task(conn, "t1"), _rules(conn, "Conference"), today) == 3


class TestDerivedStates:
    def test_important_at_high_importance(self, conn):
        _seed_task(conn, "t1", importance=3)
        t = db.get_task(conn, "t1")
        assert derived_state.is_important(t, {})
        assert not derived_state.is_urgent(t, {}, date(2026, 1, 10))

    def test_urgent_at_high_urgency(self, conn):
        _seed_task(conn, "t1", urgency=3)
        t = db.get_task(conn, "t1")
        assert derived_state.is_urgent(t, {}, date(2026, 1, 10))
        assert not derived_state.is_important(t, {})

    def test_medium_is_neither_state(self, conn):
        _seed_task(conn, "t1", importance=2, urgency=2)
        t = db.get_task(conn, "t1")
        assert not derived_state.is_important(t, {})
        assert not derived_state.is_urgent(t, {}, date(2026, 1, 10))


def _request(path="/tasks"):
    from starlette.requests import Request

    return Request(
        {
            "type": "http",
            "method": "GET",
            "path": path,
            "query_string": b"",
            "scheme": "http",
            "server": ("testserver", 80),
            "root_path": "",
            "headers": [],
        }
    )


class TestVirtualStateFilters:
    """Slice 2: the temporal/derived virtual states surface as query-driven
    projections in the Tasks Table/Board/Timeline filters -- tomorrow,
    this_month, important, urgent -- computed from derived values, never
    stored, never labels."""

    def test_table_tomorrow_filter(self, conn):
        today = date.today()
        tomorrow_iso = (today + timedelta(days=1)).isoformat()
        _seed_task(conn, "due_tomorrow", due_at=tomorrow_iso)
        _seed_task(conn, "due_today", due_at=today.isoformat())
        resp = tasks_router.list_tasks(_request(), date_filter="tomorrow", conn=conn)
        open_uids = {t["uid"] for t in resp.context["open_tasks"]}
        assert open_uids == {"due_tomorrow"}

    def test_table_this_month_filter(self, conn):
        today = date.today()
        first = today.replace(day=1)
        month_end = (first + timedelta(days=35)).replace(day=1) - timedelta(days=1)
        _seed_task(conn, "in_month", due_at=first.isoformat())
        _seed_task(conn, "end_month", due_at=month_end.isoformat())
        _seed_task(conn, "next_month", due_at=(month_end + timedelta(days=1)).isoformat())
        resp = tasks_router.list_tasks(_request(), date_filter="this_month", conn=conn)
        open_uids = {t["uid"] for t in resp.context["open_tasks"]}
        assert open_uids == {"in_month", "end_month"}

    def test_important_filter_is_derived_not_label(self, conn):
        db.upsert_label_config(conn, {"name": "Exam", "importance": 3, "created_at": _now()})
        _seed_task(conn, "important_exam", tags=["Exam"])
        _seed_task(conn, "untagged", tags=[])
        _seed_task(conn, "explicit_important", importance=3)
        resp = tasks_router.list_tasks(_request(), date_filter="important", conn=conn)
        open_uids = {t["uid"] for t in resp.context["open_tasks"]}
        assert open_uids == {"important_exam", "explicit_important"}

    def test_urgent_filter_uses_explicit_urgency(self, conn):
        _seed_task(conn, "explicit_urgent", urgency=3)
        _seed_task(conn, "normal", urgency=1)
        resp = tasks_router.list_tasks(_request(), date_filter="urgent", conn=conn)
        open_uids = {t["uid"] for t in resp.context["open_tasks"]}
        assert open_uids == {"explicit_urgent"}

    def test_urgent_filter_uses_label_threshold_and_time(self, conn):
        db.upsert_label_config(conn, {"name": "Conference", "urgency_threshold_days": 7, "created_at": _now()})
        today = date.today()
        _seed_task(conn, "conf_soon", due_at=(today + timedelta(days=2)).isoformat(), tags=["Conference"])
        _seed_task(conn, "conf_later", due_at=(today + timedelta(days=30)).isoformat(), tags=["Conference"])
        _seed_task(conn, "overdue", due_at=(today - timedelta(days=1)).isoformat())
        resp = tasks_router.list_tasks(_request(), date_filter="urgent", conn=conn)
        open_uids = {t["uid"] for t in resp.context["open_tasks"]}
        assert open_uids == {"conf_soon", "overdue"}

    def test_important_and_urgent_are_not_labels_in_data(self, conn):
        # The virtual states never create object_labels rows or label_config
        # entries -- filtering by them is a pure projection.
        _seed_task(conn, "t1", importance=3, tags=["Exam"])
        tasks_router.list_tasks(_request(), date_filter="important", conn=conn)
        tasks_router.list_tasks(_request(), date_filter="urgent", conn=conn)
        labels = db.list_all_label_names(conn)
        assert "important" not in {l.lower() for l in labels}
        assert "urgent" not in {l.lower() for l in labels}
        assert "tomorrow" not in {l.lower() for l in labels}
        assert "this month" not in {l.lower() for l in labels}

    def test_board_and_timeline_respect_virtual_filters(self, conn):
        today = date.today()
        _seed_task(conn, "imp", status="active", importance=3)
        _seed_task(conn, "not_imp", status="active")
        board = tasks_router.board_view(_request("/tasks/board"), date_filter="important", conn=conn)
        board_uids = {t["uid"] for col in board.context["columns"].values() for t in col}
        assert board_uids == {"imp"}
        _seed_task(conn, "imp2", status="active", importance=3, due_at=today.isoformat())
        tl = timeline_router.timeline_view(_request("/tasks/timeline"), date_filter="important", conn=conn)
        tl_uids = {b["task"]["uid"] for b in tl.context["bars"]}
        assert "imp2" in tl_uids

    def test_tomorrow_and_this_month_are_in_the_toolbar_dropdown(self, conn):
        resp = tasks_router.list_tasks(_request(), conn=conn)
        body = resp.body.decode()
        assert 'value="tomorrow"' in body
        assert 'value="this_month"' in body
        assert 'value="important"' in body
        assert 'value="urgent"' in body


class TestAggregationService:
    """Slice 3: src/derived_state.py's count_by_state -- the one shared
    aggregation service every surface reads instead of re-implementing the
    math. Counts must agree with the Tasks page's virtual-state filters by
    construction (same predicate), which the cross-check below asserts."""

    def _open_tasks(self, conn):
        return db.list_tasks(conn)

    def test_counts_every_state_in_one_pass(self, conn):
        today = date.today()
        _seed_task(conn, "overdue", due_at=(today - timedelta(days=1)).isoformat())
        _seed_task(conn, "today", due_at=today.isoformat())
        _seed_task(conn, "tomorrow", due_at=(today + timedelta(days=1)).isoformat())
        _seed_task(conn, "in_week", due_at=(today + timedelta(days=3)).isoformat())
        _seed_task(conn, "next_month", due_at=(today.replace(day=28) + timedelta(days=15)).isoformat())
        _seed_task(conn, "imp", importance=3)
        _seed_task(conn, "urg", urgency=3)
        counts = derived_state.count_by_state(self._open_tasks(conn), db.list_label_rules(conn))
        assert counts["overdue"] == 1
        assert counts["today"] == 1
        assert counts["tomorrow"] == 1
        assert counts["this_week"] == 3  # today + tomorrow + in_week
        assert counts["important"] == 1
        # Urgent = explicit urgent task + the overdue one + the due-today one
        # (temporal urgency), because a task counts toward every state it
        # belongs to.
        assert counts["urgent"] == 3

    def test_counts_match_filters_exactly(self, conn):
        today = date.today()
        _seed_task(conn, "a", due_at=today.isoformat(), importance=3)
        _seed_task(conn, "b", due_at=(today + timedelta(days=3)).isoformat(), urgency=3)
        _seed_task(conn, "c", due_at=(today - timedelta(days=2)).isoformat())
        tasks = db.list_tasks(conn)
        rules = db.list_label_rules(conn)
        counts = derived_state.count_by_state(tasks, rules)
        # For every state, the aggregation count equals the filter result set
        # size -- the single source of truth guarantees the agreement.
        for state, n in counts.items():
            resp = tasks_router.list_tasks(_request(), date_filter=state, conn=conn)
            filtered = resp.context["open_tasks"]
            assert len(filtered) == n, f"state {state}: filter {len(filtered)} != aggregate {n}"

    def test_label_rule_fed_counts(self, conn):
        db.upsert_label_config(conn, {"name": "Exam", "importance": 3, "created_at": _now()})
        today = date.today()
        _seed_task(conn, "exam", due_at=(today + timedelta(days=2)).isoformat(), tags=["Exam"])
        counts = derived_state.count_by_state(self._open_tasks(conn), db.list_label_rules(conn))
        assert counts["important"] == 1
        assert counts["this_week"] == 1


class TestAtAGlanceWidget:
    def test_renderer_uses_aggregation_service(self, conn):
        from src.routers import dashboard as dashboard_router

        today = date.today()
        _seed_task(conn, "overdue", due_at=(today - timedelta(days=1)).isoformat())
        _seed_task(conn, "today", due_at=today.isoformat())
        _seed_task(conn, "imp", importance=3)
        data = dashboard_router._render_at_a_glance(conn, {})
        assert data["overdue_count"] == 1
        assert data["today_count"] == 1
        assert data["week_count"] == 1
        assert data["important_count"] == 1
        # The overdue and due-today tasks are both urgent (temporal urgency).
        assert data["urgent_count"] == 2
        assert data["overdue_link"].startswith("/tasks?date_filter=overdue")

    def test_at_a_glance_widget_renders_five_stats(self, conn):
        from src.routers import dashboard as dashboard_router

        today = date.today()
        _seed_task(conn, "overdue", due_at=(today - timedelta(days=1)).isoformat())
        _seed_task(conn, "imp", importance=3)
        data = dashboard_router._render_at_a_glance(conn, {})
        ctx = {"request": _request(), "data": data}
        body = dashboard_router.templates.get_template("_widget_at_a_glance.html").render(ctx)
        assert "Overdue" in body
        assert "Due today" in body
        assert "Due this week" in body
        assert "Important" in body
        assert "Urgent" in body


class TestWebDAVRepresentation:
    """Slice 4: Importance/Urgency -> iCal PRIORITY in ical_rows.py. The
    recorded decision: PRIORITY carries the combined axes via an
    urgency-dominant precedence table; import maps PRIORITY back to explicit
    urgency only (this app is the write-source)."""

    def test_urgency_dominant_export(self):
        # Urgency maps to the low/red iCal PRIORITY numbers.
        assert ical_rows._priority_to_ical(None, 3) == 1
        assert ical_rows._priority_to_ical(1, 2) == 3
        assert ical_rows._priority_to_ical(None, 1) == 5

    def test_importance_fills_between_values_when_urgency_unset(self):
        assert ical_rows._priority_to_ical(3, None) == 2
        assert ical_rows._priority_to_ical(2, None) == 4
        assert ical_rows._priority_to_ical(1, None) == 6

    def test_both_unset_is_zero(self):
        assert ical_rows._priority_to_ical(None, None) == 0
        assert ical_rows._priority_to_ical(0, 0) == 0

    def test_import_maps_priority_to_urgency_only(self):
        assert ical_rows._priority_from_ical(1) == 3
        assert ical_rows._priority_from_ical(2) == 3
        assert ical_rows._priority_from_ical(3) == 2
        assert ical_rows._priority_from_ical(5) == 2
        assert ical_rows._priority_from_ical(6) == 1
        assert ical_rows._priority_from_ical(9) == 1
        assert ical_rows._priority_from_ical(None) is None
        assert ical_rows._priority_from_ical(0) is None

    def test_round_trip_preserves_urgency(self):
        row = {"uid": "t1", "title": "T", "due_at": "2026-08-10", "importance": 3, "urgency": 2, "status": "active"}
        todo = Todo.from_ical(task_row_to_ical(row))
        result = ical_to_task_row(todo)
        assert result["urgency"] == 2  # urgency round-trips exactly
        assert "importance" not in result or result.get("importance") is None

    def test_unset_round_trip_stays_unset(self):
        row = {"uid": "t1", "title": "T", "status": "active"}
        todo = Todo.from_ical(task_row_to_ical(row))
        result = ical_to_task_row(todo)
        assert result.get("urgency") is None

    def test_tasks_csv_exports_importance_and_urgency(self, conn):
        from src.routers import export as export_router

        _seed_task(conn, "t1", importance=3, urgency=2)
        resp = export_router.export_tasks_csv(conn=conn)
        body = resp.body.decode()
        assert "Importance" in body
        assert "Urgency" in body
        assert "Priority" not in body
        row = [c for c in body.splitlines()[1].split(",")]
        assert row[4] == "3"  # importance column populated
        assert row[5] == "2"  # urgency column populated


class TestSlice5AxesInUI:
    """Slice 5: the two axes replace the old single priority everywhere in
    the UI -- independent filters, sort keys, form storage, inline edits,
    and table/board/detail rendering."""

    def _json_request(self, payload):
        import asyncio
        import json as _json
        from starlette.requests import Request

        req = Request({"type": "http", "method": "POST", "path": "/x",
                       "headers": [(b"content-type", b"application/json")]})

        async def receive():
            return {"type": "http.request", "body": _json.dumps(payload).encode(), "more_body": False}

        req._receive = receive
        return req

    def test_active_filter_count_counts_both_axes(self):
        assert tasks_router._active_filter_count("all", "all", "all", "all", None) == 0
        assert tasks_router._active_filter_count("all", "all", "3", "all", None) == 1
        assert tasks_router._active_filter_count("all", "all", "all", "2", None) == 1
        assert tasks_router._active_filter_count("all", "all", "3", "2", None) == 2

    def test_sort_keys_have_importance_and_urgency(self):
        assert "importance" in tasks_router._SORT_KEYS
        assert "urgency" in tasks_router._SORT_KEYS
        assert "priority" not in tasks_router._SORT_KEYS
        # Higher = first; unset sorts as 0.
        assert tasks_router._SORT_KEYS["importance"]({"importance": 3}) > tasks_router._SORT_KEYS["importance"]({"importance": None})
        assert tasks_router._SORT_KEYS["urgency"]({"urgency": None}) == 0

    def test_updatable_fields_use_both_axes_not_priority(self):
        assert "importance" in tasks_router._UPDATABLE_FIELDS
        assert "urgency" in tasks_router._UPDATABLE_FIELDS
        assert "priority" not in tasks_router._UPDATABLE_FIELDS

    def test_update_field_coerces_importance_and_urgency_ints(self, conn):
        import asyncio

        _seed_task(conn, "t1")
        asyncio.run(tasks_router.update_field("t1", self._json_request({"field": "importance", "value": "3"}), conn=conn))
        asyncio.run(tasks_router.update_field("t1", self._json_request({"field": "urgency", "value": "2"}), conn=conn))
        t = db.get_task(conn, "t1")
        assert t["importance"] == 3
        assert t["urgency"] == 2

    def test_update_field_clears_axes_with_empty_string(self, conn):
        import asyncio

        _seed_task(conn, "t1", importance=3, urgency=2)
        asyncio.run(tasks_router.update_field("t1", self._json_request({"field": "importance", "value": ""}), conn=conn))
        asyncio.run(tasks_router.update_field("t1", self._json_request({"field": "urgency", "value": ""}), conn=conn))
        t = db.get_task(conn, "t1")
        assert t["importance"] is None
        assert t["urgency"] is None

    def test_update_field_rejects_priority(self, conn):
        import asyncio

        _seed_task(conn, "t1")
        resp = asyncio.run(tasks_router.update_field("t1", self._json_request({"field": "priority", "value": "1"}), conn=conn))
        assert resp.status_code == 400

    def test_list_filters_by_importance_and_urgency_independently(self, conn):
        _seed_task(conn, "a", importance=3, urgency=1)
        _seed_task(conn, "b", importance=1, urgency=3)
        _seed_task(conn, "c", importance=3, urgency=3)
        resp = tasks_router.list_tasks(_request(), importance_filter="3", conn=conn)
        assert {t["uid"] for t in resp.context["open_tasks"]} == {"a", "c"}
        resp = tasks_router.list_tasks(_request(), urgency_filter="3", conn=conn)
        assert {t["uid"] for t in resp.context["open_tasks"]} == {"b", "c"}
        resp = tasks_router.list_tasks(_request(), importance_filter="3", urgency_filter="3", conn=conn)
        assert {t["uid"] for t in resp.context["open_tasks"]} == {"c"}

    def test_table_renders_both_pill_select_columns(self, conn):
        _seed_task(conn, "t1", importance=3, urgency=2)
        resp = tasks_router.list_tasks(_request(), conn=conn)
        ctx = {"request": _request(), **resp.context}
        body = tasks_router.templates.get_template("tasks_list.html").render(ctx)
        assert 'data-field="importance"' in body
        assert 'data-field="urgency"' in body
        assert "Importance" in body
        assert "Urgency" in body

    def test_board_renders_both_pills(self, conn):
        _seed_task(conn, "t1", importance=3, urgency=2, status="active")
        resp = tasks_router.board_view(_request(), conn=conn)
        ctx = {"request": _request(), **resp.context}
        body = tasks_router.templates.get_template("tasks_board.html").render(ctx)
        assert 'title="Importance"' in body
        assert 'title="Urgency"' in body
        # Task has importance 3 and urgency 2 -- both pills render.
        assert "High" in body
        assert "Medium" in body

    def test_detail_renders_both_meta_items(self, conn):
        _seed_task(conn, "t1", importance=3, urgency=2, status="active")
        resp = tasks_router.task_detail("t1", _request(), conn=conn)
        ctx = {"request": _request(), **resp.context}
        body = tasks_router.templates.get_template("task_detail.html").render(ctx)
        assert "<span class=\"detail-meta-label\">Importance</span>" in body
        assert "<span class=\"detail-meta-label\">Urgency</span>" in body
        assert "High" in body
        assert "Medium" in body

    def test_quick_add_offers_both_axes(self, conn):
        from src.routers import dashboard as dashboard_router

        resp = dashboard_router.quick_add_form(_request(), conn=conn)
        body = resp.body.decode()
        assert 'data-ms-label="importance"' in body or 'name="importance"' in body
        assert 'data-ms-label="urgency"' in body or 'name="urgency"' in body