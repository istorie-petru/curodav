...
## Spaces — labels-as-membership rework (scoped 2026-09-14)

**Status:** scoped 2026-09-14, direct request. Reverses part of a decision
`open-priority.md`'s "Spaces — context" section marked **confirmed shipped
2026-08-14**: today a Space's page (`routers/spaces.py::_label_scope`,
`routers/labels.py::_label_scope`'s `generate_space` branch) shows only
items *directly* tagged with the Space's own label name — the exact
"manually assign the group label" pattern this rework retires. Confirmed
against the actual code before scoping, not assumed: there is no `groups`
table. A Space is already a `label_config` row with `generate_space=1`
(`db.py` ~line 434). Two separate, currently disconnected "group"
mechanisms exist on every other label: `parent_name` (a real FK column,
read by `db.list_child_labels` and by the widget grid's own
`_child_label_names` helper, `dashboard.py:204` — its docstring already
describes this exact target design — but **unwritten by any template
today**, the only writer being the orphaned `POST /settings/labels/
{name}/set` route whose own docstring says so); and `label_group`
(`_label_form_fields.html:43`, a free-text `<input>`, "e.g. Work,
Priority" — saved by `labels.py::create_label`/`update_label`, read only
for Settings-table sorting/badge display, zero effect on aggregation or
any Space page). This rework makes `parent_name` the one real link and
retires `label_group`'s grouping role.

Confirmed *not* a bug, contrary to the report that prompted checking it:
the label-icon-tile background (`.label-icon-tile`, style.css) already
reads `label.color` everywhere it's used today (`_page_banner.html:144`,
`label_detail.html`, `project_detail.html`) — the only place it needs to
newly apply is the per-Space section heading slice 2 below adds, which is
forward requirement, not a fix.

**Acceptance:** every plain label and project belongs to at most one
Space, set via a dropdown (not free text) in the label edit form. Settings
> Labels renders one table per Space (plus one "Ungrouped" table) instead
of one flat table with a text badge. A Space's page shows every task/
event/contact carrying *any* label that belongs to that Space (e.g.
University's page shows items tagged `Historiography` or `1st Semester`,
not items tagged `University` directly) — Space labels can no longer be
assigned to an item themselves. The Dashboard's per-Space widget grid
applies this as one hard, top-level filter instead of each widget
independently re-deriving it. The Upcoming widget shows both tasks and
events. Existing direct Space-label tags are migrated away, not left as
dead-but-readable data.

**Slices** (dependency order; 1→2 are Settings/data-entry, 3 is the
aggregation-semantics flip 4-5 build on, 6 is cleanup and can run anytime
after 3):

1. **Label edit form: Space-link dropdown replaces free-text Group.**
   `_label_form_fields.html:43`'s `<input name="label_group">` becomes
   `<select name="parent_name">`, populated from existing
   `generate_space=1` labels plus an empty "No space" option; hidden
   entirely when editing a Space itself (Spaces don't nest — matches
   `list_child_labels`'s existing non-recursive, one-level design).
   `labels.py::create_label`/`update_label` (~358-515) accept
   `parent_name: str = Form("")`, validate it names an existing
   `generate_space=1` label (400 otherwise, this router's existing
   validation style), write `label_config.parent_name`. Stop writing
   `label_group` going forward; leave the column and any existing values
   alone (slice 6 decides its fate). The Settings-table sort key
   (`labels.py:282`) switches from `label_group` to `parent_name`.

2. **Settings > Labels: one table per Space.** `labels.py::_labels_context`
   groups labels by `parent_name` into `{space_name: [...]}` plus an
   "Ungrouped" bucket instead of one flat sorted list.
   `labels_manage.html`/`_labels_table_body.html` render one `<table>` per
   Space (heading = Space name) plus the Ungrouped table, replacing the
   flat table + group-badge column. Each Space section heading gets a
   colored icon tile matching `.label-icon-tile`'s existing pattern
   (`--tile-swatch:var(--cal-bg-{{ space.color }})`) — the one place this
   rework actually adds a new icon-tile, per the "confirmed not a bug"
   note above. Existing per-row icon/color treatment (shipped 2026-09-14)
   is unchanged.

3. **Space pages aggregate by membership, not direct tagging.**
   `routers/spaces.py::_label_scope` (lines 20-30) and
   `routers/labels.py::_label_scope`'s `generate_space` branch drop their
   `name in tags` direct-membership filter and instead fold in every
   child label's name, reusing `dashboard.py::_child_label_names`/
   `db.list_child_labels` — the same helper the widget grid already calls
   internally, not a new implementation. Update `open-priority.md`'s
   "Spaces — context" section: mark its "direct `object_labels`
   membership only, never transitive through `parent_name`" line as
   superseded 2026-09-14, pointing here.

4. **Dashboard: hard top-level filter.** Today
   `routers/dashboard.py::widget_page_context` (line 1868) leaves each
   widget's own render function to independently call
   `_child_label_names`/`list_child_labels` and fold the result into its
   own query — a soft, per-widget filter a new widget type could forget
   to apply. Compute the scope's `child_label_names` once at the top of
   `widget_page_context` when `project_uid`/`space_uid` is set and thread
   it down so every widget's underlying task/event/contact query starts
   pre-filtered to that set, before any per-widget logic runs.

5. **Upcoming widget shows tasks and events.** `dashboard.py:1447`'s
   `_DEFAULT_STACK_MEMBER_TYPES` entry
   `("agenda", {"range": "all_upcoming", "show": ["events"]}, "Upcoming")`
   → `"show": ["events", "tasks"]`. Config-only: the agenda widget's
   `show` list already supports both (used this way elsewhere, e.g.
   `dashboard.py:1686`/`1707`).

6. **Migration: strip legacy direct Space-label tagging.** New one-off
   script, same pattern as `scripts/migrate_labels.py`: for every
   `generate_space=1` label, find `object_labels` rows directly tagging
   that Space's own name and remove them (the `db.clear_label` primitive,
   scoped to just the Space's own name rather than the whole label's
   removal) — after slice 3 ships these rows are unread but not gone,
   which is dead-but-visible data (the label pill would still show
   elsewhere in the UI even though the item no longer appears on the
   Space's own page). Idempotent, safe to rerun, logs a per-Space removed
   count. `label_group`'s now-unused existing text values are left alone
   deliberately — no UI reads them after slice 2, cheapest safe option;
   only worth a follow-up if they turn out to leak somewhere UI-visible.
