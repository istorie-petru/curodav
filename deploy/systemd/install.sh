...
if [ "$WITH_RADICALE" = 1 ]; then
  ...
  # Ownership/permission fix, applied unconditionally (not just on first
  # creation): this file must be owned by $SERVICE_USER, not root, because
  # curodav-radicale.service runs as User=curodav and reads it on every
  # CalDAV/CardDAV auth attempt. Installs from before this fix created it as
  # root:root while root:root 0600 (root-only) -- Radicale itself could never
  # read its own configured user, breaking auth. Re-running install.sh now
  # heals that on any existing install too. 0600 (owner rw only) is kept so
  # only $SERVICE_USER -- the identity both curodav.service and
  # curodav-radicale.service run as -- can read or write it.
  chown "$SERVICE_USER":"$SERVICE_USER" "$CONF_DIR/radicale/users"
