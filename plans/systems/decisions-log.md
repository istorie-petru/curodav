...
**Purpose:** the record of *why* the system is shaped the way it is — condensed from the original `REWORK_PLAN.md` (2026-07-15), which specified moving the app from Flutter + FastAPI + Postgres to PySide6 + file tree + Syncthing/WebDAV. The rework is fully implemented; this file is the historical/rationale record, not a live spec — for the current design, see [`architecture.md`](architecture.md); for current features, see [`../../features/`](../../features/README.md).
...
Known open work from actually using the finished app is tracked separately in [`../tasks-ux-rework.md`](../tasks-ux-rework.md).
